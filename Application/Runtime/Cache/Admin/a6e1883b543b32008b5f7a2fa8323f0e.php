...
<table class="table table-hover common_table">
		<tr>
		 ...
		 <?php if(is_array($navlist)): $i = 0; $__LIST__ = $navlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$navitem): $mod = ($i % 2 );++$i;?><tr>
			 	<td>
			 		<input type="checkbox" class="sub_check" name="sub_check" value="<?php echo ($navitem["id"]); ?>">
			 	</td>
			 	<td>
			 		<?php echo ($navitem["id"]); ?>
			 	</td>
			 	<td>
			 		<?php echo ($navitem["sort"]); ?>
			 	</td>
			 	<td>
			 		<?php echo ($navitem["type_name"]); ?>
			 	</td> 
				<td>
			 		<?php if($navitem["status"] == 1): ?>显示
			 			<?php else: ?>
			 	 		隐藏<?php endif; ?>
			 	</td> 
