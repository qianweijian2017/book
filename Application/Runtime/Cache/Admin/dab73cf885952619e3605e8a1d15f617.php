...
<div id="main" class="main">
 ...
 <!-- 用户页面的搜索end -->
<!-- 用户列表显示开始 -->
<table class="table table-hover common_table" id="book-content">
		<tr>
			<th class="select">
		 ...
		 <?php if(is_array($booklist)): $i = 0; $__LIST__ = $booklist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bookitem): $mod = ($i % 2 );++$i;?><tr>
			 	<td>
			 		<input type="checkbox" class="sub_check" name="sub_check" value="<?php echo ($bookitem["id"]); ?>">
			 	</td>
			 	<td>
			 		<?php echo ($bookitem["id"]); ?>
			 	</td>
			 	<td>
			 		<?php echo (mb_substr($bookitem["book_name"],0,25)); ?>
			 	</td> 
				<td>
			 		<?php echo ($bookitem["bk_menu"]); ?>
			 	</td> 
			 	 <td>
			 		<?php echo ($bookitem["book_sale"]); ?>
			 	</td>
			 	<td>
			 		<?php echo ($bookitem["book_prince"]); ?>
			 	</td>  
			 	<td>
			 		<?php echo ($bookitem["book_publish"]); ?>
			 	</td> 
			 	<td>
			 		<?php echo (date("Y-m-d H:i:s",$bookitem["book_up_time"])); ?>
			 	</td>
			 	<td>
			 		<?php echo ($bookitem["book_collect"]); ?>
			 	</td>
